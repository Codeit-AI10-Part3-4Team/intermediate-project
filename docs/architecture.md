...
- **프론트엔드 (웹 UI, `frontend/`)** — Streamlit 앱. ChatGPT식 단일 채팅 페이지: 첫 화면(질의 입력+문서 업로드 적합성 검사, `views/home.py`) ↔ 채팅 화면(답변/근거 표시, `views/chat.py`)을 `session_state`로 전환. 채팅 이력은 표시용이며 각 `/rag` 질의는 독립 호출(백엔드 대화 메모리 없음). FastAPI에 **HTTP로만** 요청하며 `rag_core`/`api`를 import하지 않음(백엔드 목업↔실제 전환에 무관). 브랜드 디자인 자산은 `frontend/design/`. VM 배포는 FE 8501만 외부 개방, API는 loopback 8090 — `deploy/systemd/rfp-*.service`·`frontend/README.md` 참고.
