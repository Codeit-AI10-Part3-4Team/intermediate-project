...
![GitHub Pull Request](https://img.shields.io/badge/GitHub-Pull_Request-181717?logo=github&logoColor=white)
![Flow](https://img.shields.io/badge/main_←_feature-2da44e?logo=github&logoColor=white)

작업 브랜치(`feature`)의 변경분을 `main`에 안전하게 합치는 표준 절차입니다.
아래 다이어그램의 흐름과 동일합니다: **feature 브랜치 생성 → 커밋 → PR 제출 → 리뷰/논의 → main으로 merge**.

<!-- 이미지 출처: GitHub Docs (공식). 경로 해시(cb-...)는 문서 갱신 시 바뀔 수 있습니다. -->
![GitHub Flow 다이어그램](https://docs.github.com/assets/cb-23923/images/help/repository/branching.png)
...
5. **수정된 내용을 push** 합니다. (push해야 GitHub 서버에 반영됩니다)
   ```bash
...
9. PR 서식([템플릿](.github/pull_request_template.md))에 **무엇을 했는지 / 어떻게 동작을 확인하는지**를 기술하고,
   오른쪽 **Reviewers**에서 **리뷰어를 수동 지정**한 뒤 제출합니다.
   - ℹ️ 추후 `CODEOWNERS` 도입 시 리뷰어가 자동 배정됩니다(현재는 수동).
...
10. 제출 후 **디스코드로 리뷰어에게 PR 요청**을 알립니다.
...
14. **승인(Approve) + CI 통과**를 확인한 뒤, 작업자가 **Merge** 합니다.
    - ⚠️ (도입 예정) 브랜치 보호 규칙상 *승인 1개 이상 + CI 통과* 전에는 Merge 버튼이 비활성화됩니다.
    - ✅ 병합 방식은 **Squash and merge** 권장(노트북 실험 커밋이 main 히스토리를 오염시키지 않도록).
